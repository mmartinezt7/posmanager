  ...
  <body onLoad="muestraReloj()">
    ...
    <div class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true" style="display: none">
                                         ...
                                         <button type="button" class="close" data-dismiss="modal" aria-hidden="true"><img src="assets/img/close.png"/></button>
                                                        <h4 class="modal-title" id="myLargeModalLabel">Detalles y Kardex del Producto </h4>
                                                    </div>
						...
						<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
						  <div class="modal-dialog">
						    <div class="modal-content">
						      <div class="modal-header">
						        <button type="button" class="close" data-dismiss="modal" aria-hidden="true"><img src="assets/img/close.png"/></button>
						        <h4 class="modal-title" id="myModalLabel">Detalles y Kardex del Producto</h4>
						      </div>
						      <div class="modal-body">
                         <div id="muestraproductomodal"></div>
						      </div>
						...
						<div class="modal fade" id="myModal2" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
						  <div class="modal-dialog">
						    <div class="modal-content">
						     ...
						     <div class="modal-body">
						 <form name="cargarproductos" id="cargarproductos" action="#" enctype="multipart/form-data">
                                                  <div id="error">
                                                 <!-- error will be shown here ! -->
                                                     </div>
                                             <div class="col-md-12"> 
                                                      <div class="form-group"> 
           <label for="field-12" class="control-label">Cargar Archivo (CSV):</label> <a href="#" data-container="body" title="" data-toggle="popover" data-placement="right" data-content="Una vez realizada la Carga de los registros exitosamente deberá de actualizar el navegador para mostrar los registros cargados" data-original-title=""><span class="fa fa-exclamation-circle"></span></a>
...
<small><p>Para realizar la Carga masiva de Productos el archivo debe de ser extensión (CSV Delimitado por Comas). Una vez realizada la Carga de los registros exitosamente deberá de actualizar el navegador para mostrar los registros cargados<br></small>
                                                                </div>
                                            </div>	
						      <div class="modal-footer">
      <button type="button" class="btn btn-danger waves-effect" data-dismiss="modal"><span class="fa fa-times"></span> Cerrar</button>
      <button type="submit" name="btn-cargar" id="btn-cargar" class="btn btn-primary waves-effect waves-light"><span class="fa fa-cloud-upload"></span> Cargar</button>
						      </div></form>
						      </div>
						    </div>
						  </div>
      ...
      *********************************************************************************************************************************************************** -->
      ...
      <section id="main-content">
          <section class="wrapper">
          	<h3><i class="fa fa-tasks"></i> Módulo General de Productos</h3><br>
			...
			<div class="row">
						    <div class="col-lg-12">
                                <div class="panel panel-border panel-warning widget-s-1">
                                    <div class="panel-heading"><h4><i class="fa fa-search"></i> Consulta General de Productos<span class="pull-right"><div class="btn-group"><button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown">
						    <span class="fa fa-print"></span> Imprimir Reportes <span class="caret"></span>
						  </button>
						  <ul class="dropdown-menu" role="menu">
<li><a href="reportepdf?tipo=<?php echo base64_encode("PRODUCTOS") ?>" target="_blank"><i class="fa fa-file-pdf-o"></i> Listado de Productos Pdf</a></li>
<li><a href="reportepdf?tipo=<?php echo base64_encode("PRODUCTOSVENDIDOS") ?>" target="_blank"><i class="fa fa-file-pdf-o"></i> Listado de Productos Vendidos</a></li>
<li><a href="productosexcel"><i class="fa fa-file-excel-o"></i> Listado de Productos Excel</a></li>
<li><a href="reportepdf?tipo=<?php echo base64_encode("STOCK") ?>" target="_blank"><i class="fa fa-file-pdf-o"></i> Productos en Stock Minimo</a></li>
						  </ul>
						</div>
		&nbsp;<div class="btn-group"><a href="forproducto" class="btn btn-primary"><span class="fa fa-cube"></span> Registrar Productos</a> </div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span></h4> </div>
		...
		if(isset($_GET["mesage"]))
{
	switch($_GET["mesage"])
	{
		...
		echo "<span class='fa fa-check-square-o'></span> EL PRODUCTO FUE ELIMINADO EXITOSAMENTE </div></center>"; 
		break;
		
		case 2:
		echo "<center><div class='alert alert-warning'>";
		echo "<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>";
		echo "<span class='fa fa-info-circle'></span> ESTE PRODUCTO NO PUEDE SER ELIMINADO, TIENE VENTAS RELACIONADAS ACTUALMENTE </div></center>";  
		break;
		
		case 3:
		echo "<center><div class='alert alert-warning'>";
		echo "<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>";
		echo "<span class='fa fa-info-circle'></span> USTED NO TIENE ACCESO PARA ELIMINAR PRODUCTOS, NO ERES EL ADMINISTRADOR DEL SISTEMA </div></center>";  
		break;
...
?>
                              <tr>
                                  ...
                                  <td>
<a href="#" class="btn btn-success btn-xs" onClick="VerProducto('<?php echo $reg[$i]['codproducto'] ?>')" data-href="#" data-toggle="modal" data-target=".bs-example-modal-lg" data-placement="left" data-backdrop="static" data-keyboard="false" data-id="" rel="tooltip" data-original-title="Ver Producto" title="Ver Producto"><i class="fa fa-search-plus"></i></a>

<!--<a href="#" title="Ver Producto" onClick="VerProducto('')" class="btn btn-success btn-xs" data-toggle="modal" data-target="#myModal" data-backdrop="static" data-keyboard="false"><i class="fa fa-search-plus"></i></a>-->
								  
<a href="#" class="btn btn-primary btn-xs" title="Editar" onClick="actualizar('editproducto?codalmacen=<?php echo base64_encode($reg[$i]["codalmacen"]) ?>')"><i class="fa fa-pencil"></i></a>
                                 
<a href="#" class="btn btn-danger btn-xs" onClick="eliminar('eliminar?<?php echo base64_encode('eliminar'); ?>&codproducto=<?php echo base64_encode($reg[$i]["codproducto"]) ?>&tipo=<?php echo base64_encode("PRODUCTOS") ?>')" title="Eliminar" ><i class="fa fa-trash-o"></i></a> 
								  </td>
                              </tr>
												<?php  }  ?>
                              </tbody>
