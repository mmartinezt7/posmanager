						  ...
						  <div class="modal-dialog">
						    <div class="modal-content">
						      <div class="modal-header">
						        ...
						        <h4 class="modal-title" id="myModalLabel">Datos del Item</h4>
						      </div>
						      <div class="modal-body">
                         <div id="muestraitemmodal"></div>
						      </div>
      ...
      *********************************************************************************************************************************************************** -->
      ...
      <section id="main-content">
          <section class="wrapper">
          	<h3><i class="fa fa-tasks"></i> Módulo General de Items para Servicios</h3><br>
			...
			<div class="row">
						    <div class="col-lg-12">
                                <div class="panel panel-border panel-warning widget-s-1">
                                    <div class="panel-heading"><h4><i class="fa fa-search"></i> Consulta General de Items para Servicios<span class="pull-right"><a href="reportepdf?tipo=<?php echo base64_encode("ITEMS") ?>" target="_blank" class="btn btn-primary"><span class="fa fa-print"></span> Imprimir Listado</a>&nbsp;<a href="foritem" class="btn btn-primary"><span class="fa fa-plus"></span> Nuevo Items</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span></h4> </div>
		...
		if(isset($_GET["mesage"]))
{
	switch($_GET["mesage"])
	{
		...
		echo "<span class='fa fa-check-square-o'></span> EL ITEMS PARA SERVICIOS FUE ELIMINADO EXITOSAMENTE </div></center>"; 
		break;
		
		case 2:
		echo "<center><div class='alert alert-warning'>";
		echo "<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>";
		echo "<span class='fa fa-info-circle'></span> ESTE ITEMS PARA SERVICIOS NO PUEDE SER ELIMINADO, TIENE FACTURAS RELACIONADAS ACTUALMENTE </div></center>";  
		break;
		
		case 3:
		echo "<center><div class='alert alert-warning'>";
		echo "<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>";
		echo "<span class='fa fa-info-circle'></span> USTED NO TIENE ACCESO PARA ELIMINAR ITEMS PARA SERVICIOS, NO ERES EL ADMINISTRADOR DEL SISTEMA </div></center>";  
		break;
...
?>
                              <tr>
                                  <td><?php echo $a++; ?></td>
                                  <td><?php echo $reg[$i]['coditems']; ?></td>
                                  <td><?php echo $reg[$i]['nombreitems']; ?></td>
                                  <td><?php echo number_format($reg[$i]['costoitems'], 2, '.', '.'); ?></td>
                                  <td><?php echo $status = ( $reg[$i]['statusitems'] == 'ACTIVO' ? "<span class='label label-success'><i class='fa fa-check'></i> ACTIVO</span>" : "<span class='label label-warning'><i class='fa fa-times'></i> INACTIVO</span>"); ?></td>
                                  <td>
<a href="#" title="Ver Items" onClick="VerItems('<?php echo $reg[$i]['iditems'] ?>')" class="btn btn-success btn-xs" data-toggle="modal" data-target="#myModal" data-backdrop="static" data-keyboard="false"><i class="fa fa-search-plus"></i></a>
								  
<a href="#" class="btn btn-primary btn-xs" title="Editar" onClick="actualizar('edititem?iditems=<?php echo base64_encode($reg[$i]["iditems"]) ?>')"><i class="fa fa-pencil"></i></a>
                                 
<a href="#" class="btn btn-danger btn-xs" onClick="eliminar('eliminar?<?php echo base64_encode('eliminar'); ?>&iditems=<?php echo base64_encode($reg[$i]["iditems"]) ?>&tipo=<?php echo base64_encode("ITEMS") ?>')" title="Eliminar" ><i class="fa fa-trash-o"></i></a> 
								  </td>
                              </tr>
												<?php  }  ?>
                              </tbody>
