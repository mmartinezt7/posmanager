...
$reg = $tra->ListarDetallesDevoluciones();
						  ...
						  <div class="modal-dialog">
						    <div class="modal-content">
						      <div class="modal-header">
						        ...
						        <h4 class="modal-title" id="myModalLabel">Datos de Detalle de Devolución de Productos</h4>
						      </div>
						      <div class="modal-body">
                         <div id="muestradetalledevolucionmodal"></div>
						      </div>
      ...
      *********************************************************************************************************************************************************** -->
      ...
      <section id="main-content">
          <section class="wrapper">
          	<h3><i class="fa fa-tasks"></i> Módulo General de Detalles de Devolución de Productos</h3><br>
			...
			<div class="row">
						    <div class="col-lg-12">
                                <div class="panel panel-border panel-warning widget-s-1">
                                    <div class="panel-heading"><h4><i class="fa fa-search"></i> Consulta General de Detalles de Devolución de Productos</h4> </div>
		...
		if(isset($_GET["mesage"]))
{
	switch($_GET["mesage"])
	{
		...
		echo "<span class='fa fa-check-square-o'></span> EL DETALLE DE DEVOLUCIÓN DE PRODUCTO FUE ELIMINADO EXITOSAMENTE </div></center>"; 
		break;
		
		case 2:
		echo "<center><div class='alert alert-warning'>";
		echo "<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>";
		echo "<span class='fa fa-info-circle'></span> USTED NO TIENE ACCESO PARA ELIMINAR DETALLES DE DEVOLUCIÓN DE PRODUCTOS, NO ERES EL ADMINISTRADOR DEL SISTEMA </div></center>";  
		break;
...
?>
                              <tr>
                                  <td><?php echo $a++; ?></td>
                                  <td><?php echo $reg[$i]['coddevolucion']; ?></td>
                                  <td><?php echo $reg[$i]['codproducto']; ?></td>
                                  <td><?php echo $reg[$i]['producto']; ?></td>
                                  <td><?php echo $reg[$i]['nomcategoria']; ?></td>
                                  <td><?php echo number_format($reg[$i]['preciodevolucion'], 2, '.', ','); ?></td>
                                  <td><?php echo $reg[$i]['cantdevolucion']; ?></td>
                                  <td>
<a href="#" title="Ver Detalle de Devolución" data-id="<?php echo $reg[$i]['coddetalledevolucion']; ?>" class="btn btn-success btn-xs verdetalledevolucion" data-toggle="modal" data-target="#myModal" data-backdrop="static" data-keyboard="false"><i class="fa fa-search-plus"></i></a>

<a href="#" class="btn btn-primary btn-xs" title="Editar" onClick="actualizar('editdetalledevolucion?coddetalledevolucion=<?php echo base64_encode($reg[$i]["coddetalledevolucion"]) ?>')"><i class="fa fa-pencil"></i></a>
                                 
<a href="#" class="btn btn-danger btn-xs" onClick="eliminar('eliminar?<?php echo base64_encode('eliminar'); ?>&coddetalledevolucion=<?php echo base64_encode($reg[$i]["coddetalledevolucion"]) ?>&coddevolucion=<?php echo base64_encode($reg[$i]["coddevolucion"]) ?>&codproducto=<?php echo base64_encode($reg[$i]["codproducto"]) ?>&cantdevolucion=<?php echo base64_encode($reg[$i]["cantdevolucion"]) ?>&tipo=<?php echo base64_encode("DETALLESDEVOLUCIONES") ?>')" title="Eliminar" ><i class="fa fa-trash-o"></i></a> 							  </td>
                              </tr>
												<?php  }  ?>
                              </tbody>
                          </table>
