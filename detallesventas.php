						  ...
						  <div class="modal-dialog">
						    <div class="modal-content">
						      <div class="modal-header">
						        ...
						        <h4 class="modal-title" id="myModalLabel">Datos de Detalle de Venta de Productos</h4>
						      </div>
      ...
      *********************************************************************************************************************************************************** -->
      ...
      <section id="main-content">
          <section class="wrapper">
          	<h3><i class="fa fa-tasks"></i> Módulo General de Detalles de Ventas de Productos</h3><br>
			...
			<div class="row">
						    <div class="col-lg-12">
                                <div class="panel panel-border panel-warning widget-s-1">
                                    <div class="panel-heading"><h4><i class="fa fa-search"></i> Consulta General de Detalles de Ventas de Productos</h4> </div>
		...
		if(isset($_GET["mesage"]))
{
	switch($_GET["mesage"])
	{
		...
		echo "<span class='fa fa-info-circle'></span> ESTE DETALLE DE VENTA NO PUEDE SER ELIMINADO, SE ENCUENTRA INACTIVO PARA ELIMINAR </div></center>"; 
		break;
		
		case 2:
		echo "<center><div class='alert alert-info'>";
		echo "<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>";
		echo "<span class='fa fa-check-square-o'></span> EL DETALLE DE VENTA DE PRODUCTO FUE ELIMINADO EXITOSAMENTE </div></center>"; 
		break;
		
		case 2:
		echo "<center><div class='alert alert-warning'>";
		echo "<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>";
		echo "<span class='fa fa-info-circle'></span> USTED NO TIENE ACCESO PARA ELIMINAR DETALLES DE VENTAS DE PRODUCTOS, NO ERES EL ADMINISTRADOR DEL SISTEMA </div></center>";  
		break;
...
?>
                              <tr>
                                  ...
                                  <td>
<a href="#" title="Ver Detalle de Venta" onClick="VerDetalleVentas('<?php echo $reg[$i]['coddetalleventa'] ?>')" class="btn btn-success btn-xs" data-toggle="modal" data-target="#myModal" data-backdrop="static" data-keyboard="false"><i class="fa fa-search-plus"></i></a>

<a href="#" class="btn btn-primary btn-xs" title="Editar" onClick="actualizar('editdetalleventas?coddetalleventa=<?php echo base64_encode($reg[$i]["coddetalleventa"]) ?>')"><i class="fa fa-pencil"></i></a>
                                 
<a href="#" class="btn btn-danger btn-xs" onClick="eliminar('eliminar?<?php echo base64_encode('eliminar'); ?>&coddetalleventa=<?php echo base64_encode($reg[$i]["coddetalleventa"]) ?>&codventa=<?php echo base64_encode($reg[$i]["codventa"]) ?>&codproducto=<?php echo base64_encode($reg[$i]["codproducto"]) ?>&cantventa=<?php echo base64_encode($reg[$i]["cantventa"]) ?>&ivaproducto=<?php echo base64_encode($reg[$i]["ivaproducto"]) ?>&tipo=<?php echo base64_encode("DETALLESVENTAS") ?>')" title="Eliminar" ><i class="fa fa-trash-o"></i></a> 							  </td>
                              </tr>
												<?php  }  ?>
                              </tbody>
                          </table>
